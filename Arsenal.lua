--LIBRARY START
	...
	local library = {design = getgenv().design == "kali" and "kali" or "uwuware", tabs = {}, draggable = true, flags = {}, title = "arsenal", open = false, mousestate = inputService.MouseIconEnabled, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "floofyscripts_arsenal", fileext = ".uw"}
		...
		self:AddConnection(inputService.InputChanged, function(input)
			...
			if input.UserInputType.Name == "MouseMovement" then
				if self.cursor then
					...
					self.cursor.PointB = MousePos + Vector2.new(19, 8)
					self.cursor.PointC = MousePos + Vector2.new(8, 19)
					self.cursor1.PointA = MousePos
					self.cursor1.PointB = MousePos + Vector2.new(18, 7)
					self.cursor1.PointC = MousePos + Vector2.new(7, 18)
				end
				if self.slider then
